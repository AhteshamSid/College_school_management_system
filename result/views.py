...
def mark_entry(request):
    ...
    select_class = request.GET.get('select_class', None)
    if select_class:
        cls = ClassRegistration.objects.get(id=select_class).class_name
...
def mark_table(request, pk):
    form = SubjectRegistration.objects.get(id=pk)
    context = {'form': form}
    return render(request, 'result/mark-table.html', context)
