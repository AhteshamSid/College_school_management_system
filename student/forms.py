...
class EnrolledStudentForm(forms.Form):
    class_name = forms.ModelChoiceField(queryset=ClassInfo.objects.all(), widget=forms.Select(attrs={'class': 'form-control'}))
...
class SearchEnrolledStudentForm(forms.Form):
    reg_class = forms.ModelChoiceField(required=False, queryset=ClassRegistration.objects.all())
