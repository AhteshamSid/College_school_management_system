...
def student_list(request):
    ...
    if class_info:
        student = AcademicInfo.objects.filter(class_info=class_info)
    if registration_no:
        student = AcademicInfo.objects.filter(registration_no=registration_no)
    context = {
        'forms': forms,
        'student': student
    }
...
def enrolled_student(request):
    student = AcademicInfo.objects.exclude(status='enrolled')
    forms = EnrolledStudentForm()
    cls = request.GET.get('class_name')
    if cls:
        student = AcademicInfo.objects.exclude(status='enrolled').filter(class_info=cls)
    context = {
        'forms': forms,
        'student': student
    }
...
def enrolled_student_list(request):
    ...
    if roll_no:
        student = EnrolledStudent.objects.filter(roll=roll_no)
    context = {
        'forms': forms,
        'student': student
    }
